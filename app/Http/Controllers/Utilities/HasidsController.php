...
interface HashidsInterfaceController
{
    /**
...
use App\Http\Controllers\Utilities\MathController;

class HasidsController extends Controller implements HashidsInterfaceController
{
    ...
    public function encode(...$numbers)
    {
        ...
        foreach ($numbers as $i => $number) {
            $numbersHashInt += MathController::intval(MathController::mod($number, ($i + 100)));
        }
        ...
        foreach ($numbers as $i => $number) {
            ...
            if ($i + 1 < $numbersSize) {
                $number %= (ord($last) + $i);
                $sepsIndex = MathController::intval(MathController::mod($number, strlen($this->seps)));
    ...
    public function decode($hash)
    {
        ...
        if (isset($hashBreakdown[0])) {
            ...
            foreach ($hashArray as $subHash) {
                ...
                if (MathController::greaterThan($result, PHP_INT_MAX)) {
                    $ret[] = MathController::strval($result);
                } else {
                    $ret[] = MathController::intval($result);
                }
            }
    ...
    protected function hash($input, $alphabet)
    {
        ...
        do {
            $hash = $alphabet[MathController::intval(MathController::mod($input, $alphabetLength))] . $hash;

            $input = MathController::divide($input, $alphabetLength);
        } while (MathController::greaterThan($input, 0));
    ...
    protected function unhash($input, $alphabet)
    {
        ...
        if ($inputLength && $alphabet) {
            ...
            foreach ($inputChars as $char) {
                ...
                $number   = MathController::multiply($number, $alphabetLength);
                $number   = MathController::add($number, $position);
            }
        }
